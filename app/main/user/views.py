...
def index():
    ...
    return render_template("user.html", users=users, pagination=pagination, title=u"user exists")
...
def detail(user_id):
    ...
    return render_template("user_detail.html", user=the_user,
...
def edit(user_id):
    if current_user.id == user_id or current_user.can(Permission.UPDATE_OTHERS_INFORMATION):
        ...
        return render_template('user_edit.html', form=form, user=the_user, title=u"edit")
    else:
        abort(403)
